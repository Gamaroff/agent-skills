...
- **Observation**: `grep '^decision:'` used to extract gate decision from gate YAML. Verified gate file field is `gate:` (confirmed against `docs/development/tasks/task.9.*/task.9.gate.1.platform-detection-resolver-migration.yml` which has `gate: PASS`). The `decision:` key does not exist in the gate YAML schema.
...
- `docs/development/tasks/task.9.*/task.9.gate.1.*.yml` (reference for gate field name)
...
grep -n "^gate:\|^decision:" docs/development/tasks/task.9.*/*.gate.*.yml
