...
- **CI:** GitHub Actions, six workflows in `.github/workflows/` (badge in `README.md`). `test.yml` runs `npm run format:check`, `npm test` and `npm run eval:all` on every PR and every push to `main`/`develop`. `validate.yml` runs the repo-hygiene checks (skill frontmatter, catalog freshness, skill-dependency graph, bundle freshness) — **path-filtered** to `skills/**`, `shared/resources/**` and a few named files. `shellcheck.yml` lints the 56 tracked *source* shell scripts at `--severity=warning`, unfiltered, against a pinned ShellCheck; it is a separate workflow precisely because `validate.yml`'s filter cannot see `scripts/**` or `.agents/scripts/**`. `release.yml` runs on a `v*.*.*` tag; `docs-link-check.yml` link-checks changed markdown; `branch-policy.yml` rejects a PR into `main` whose head is not `develop`, a hotfix or a release branch.
