...
- **Reading `access.tracker` from `skills-config.yaml` in the JavaScript gates** —
  [task.61](../task.61.access-mode-config-tier/task.61.access-mode-config-tier.md). The gates here
  resolve the mode from `ACCESS_TRACKER` and `AGENT_SKILLS_ACCESS_TRACKER`, most-restrictive-wins.
  A config-declared restriction reaches them only through a shell that sourced `resolve-platform.sh`
  — which is what task.52 shipped and what every pipeline path already does. QA cycles 2–5 attempted
  the config tier inline and found a high-severity divergence from `read-config.sh` in every round;
  it is a parity problem, not a feature, and it was split out by explicit decision at the loop limit.
...
| `shared/resources/jira-stage.js`, `gh-stage.js` | capture both env names in the pre-`loadDotEnv` snapshot | QA found the stage CLIs' gate was looser than the layer-1 net beneath them |
| `shared/resources/defer-mutation.js` | `resolveAccessTracker` reads both env names, most-restrictive-wins | QA found the gates read the resolver's OUTPUT (`ACCESS_TRACKER`) and not the knob an operator sets |
