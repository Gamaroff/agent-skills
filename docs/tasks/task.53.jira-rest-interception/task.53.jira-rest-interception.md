...
- [ ] `full` mode byte-identical; the existing suite green except the roster-count assertions
- [ ] Under a non-`full` mode **no non-GET reaches the network**, proven by a throwing stub
- [ ] `jira.unknown-mutation` exists as the 21st roster kind and renders in all four formats
- [ ] All 5 new kinds produce legible records — `intent` and `desired`, not a JSON blob
- [ ] An un-annotated mutation is refused and recorded, never executed
- [ ] Exactly one record per logical mutation, including across retries and the epic double-POST
- [ ] Deferred creates return the create null shape; deferred updates return the update shape
- [ ] `jsm_curl` sets `JSM_HTTP_STATUS`/`JSM_BODY` on defer; both sprint scripts complete without abort
- [ ] `jira-create-epic.js` is gated, and its exclusion from layer 1 is stated in the document
- [ ] `--json` samples updated alongside the payloads
- [ ] The resolver's "PARTIALLY ENFORCED" notice reflects what is now enforced
- [ ] The `deferred` reason is documented where a reader will hit it — a capability that refuses
...
- [ ] Step 1 — `jira.unknown-mutation` roster kind, renderer, count updates
- [ ] Step 2 — layer 1 in `makeHttp` / `http()`
- [ ] Step 3 — layer 2 at `putIssueAtomic`, `moveToBacklog`, four create-POSTs
- [ ] Step 4 — `jsm_curl` guard incl. `JSM_HTTP_STATUS` / `JSM_BODY`
- [ ] Step 5 — `--json` `reason: "deferred"` + SKILL.md samples
- [ ] Step 6 — resolver notice + `tracker-access.test.sh` §17
- [ ] Step 7 — tests, `npm run bundle`, docs
