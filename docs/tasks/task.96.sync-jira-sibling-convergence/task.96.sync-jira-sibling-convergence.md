...
`includeDescription === false` regardless, so `--force` re-published nothing. Neither half was
reachable before this task, because the label-diff defect kept the gate shut — which is why it
reads as a restoration and is not one. Operators who use `--force` to repair a card edited in the
Jira UI get the behaviour they already expected; nobody loses any.
...
- [x] `npm run bundle` — note `jira-sync.js` has **22 bundled copies (plus 4 new `fake-jira.js` copies)** under `skills/*/references/`, so any Phase 4 change to it fans out to all 23 and every one must be committed
...
4. ✅ `shared/resources/jira-sync.js` — shared helper, **only if** Phase 1 justifies it. Changing it fans out to 22 bundled `references/` copies via `npm run bundle`
...
Production code and shared library:

- `shared/resources/jira-sync.js` — `diffAgainstPayload` + export; header un-staled
- `shared/resources/fake-jira.js` — **new**, the generalised harness (vendored into 4 skills)
- `skills/sync-jira-{story,task,epic,bug}/scripts/*.js` — payload-before-diff and the helper
  migration in all four; post-transition re-read in story, task and epic; story's `!args.force`
...
- `CHANGELOG.md` — the behaviour change, and the `--force` capability, under `### Fixed`
- `skills/sync-jira-task/SKILL.md` — literal test count replaced with a description
- `skills/sync-jira-story/SKILL.md` — `--force`'s two effects documented, matching epic

Generated (committed, not hand-edited): 22 bundled `skills/*/references/jira-sync.js` and 4
bundled `skills/*/references/fake-jira.js`.
...
**Cycle 3** (after `/review-pr` returned REQUEST CHANGES) — 14 more, including two HIGH: making the
skip gate reachable meant `assignee`, `due_date`, `components` and `fix_versions` — carried by the
payload, compared by neither `diffFields` nor `hashMeta` — were **silently dropped** on story and
epic, with the run reporting success. Also corrected: `--force`'s framing (it is a new capability,
...
Three of the 17 criteria in §9 are ticked with qualifications, stated here because the traceability
matrix that records them lives under a gitignored `.summaries/` directory and never reaches a PR
reader:

- **SC9 (mutation-proof)** — every behavioural fix was reverted and its named test confirmed red,
  and QA re-ran the proofs independently. It remains an attestation: no re-runnable mutation-testing
  artifact is produced.
- **SC10 (red before green)** — the tests were confirmed red first, but the suites and the fixes
  share commit `b44e12e5`, so a reviewer cannot check out a commit where the tests exist and fail.
  Disclosed rather than fixed by rewriting published history.
