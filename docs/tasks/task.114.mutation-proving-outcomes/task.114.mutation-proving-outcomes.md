...
- `shared/resources/mutation-proving.md` — "## The six shapes vacuity takes" (≈215); false-RED
  table with a "wrong thing mutated" row (task.100); its own lint (`mutation-proving.md:191`
  anticipates a seventh shape).
- Consumers: `skills/develop/SKILL.md:≈653`, `skills/qa-story/SKILL.md:≈373`, `skills/qa-task/SKILL.md:≈430`
  (each says "the four shapes"); `qa-task` Step 3c records `mutation-proven: yes/no` with no outcome
  vocabulary.
- Bundled into five skills — edit the source, `npm run bundle`.
...
2. Rewrite the doc; keep every existing heading the lint or a test greps for (check
   `grep -rn "mutation-proving" tests evals shared/resources/tests` first).
...
Parity test (mutation-proved); existing lint over the doc still green; bundle idempotent.
...
**Low.** Prose. The one trap is the doc's own lint and any test that greps its headings — enumerate
...
- [ ] `mutation-proving.md` gains "What a mutation run can tell you" — red-the-predicted-test / red-the-wrong-test / no-red-dead / no-red-load-bearing / survivor-because-wrong-mutation / green-because-mutation-never-applied / red-only-because-of-current-data
### Phase 2: the instrument rules
- [ ] Assert the mutation applied; snapshot-restore, never `git checkout --`; baseline-green between mutations; name the expected red test first; probe-validity ("what would this print if broken?"); helper returns `{ok, value}`
### Phase 3: the corpus rules
- [ ] Seventh shape (no instance of the input class); absorbed-by-fallback → search, don't reason; a check is blind to what it doesn't iterate; committed test vs development-time proof
### Phase 4: consumers
- [ ] `qa-task`/`qa-story` Step 3c point at the table and record outcome per proof; the three "four shapes" pointers drop their count; the doc's own lint updated
...
- **Canonical**: `shared/resources/mutation-proving.md` (bundled into develop, qa-story, qa-task, review-security, finalise)
