...
| File | Change |
| ---- | ------ |
| `shared/resources/jira-sync.js` | add `toRelativeDocLink`, `makeFenceTracker`, `extractSection`; route `extractBodySections` through it; export all three |
...
| `skills/sync-jira-epic/scripts/sync-jira-epic.js` | drop two key writes; relative `**Epic File**` / `**Parent PRD**`; hoist `prdFilePath`; `extractStoriesTable` uses `extractSection`; export `updateEpicFile` |
...
- [x] The write-back functions are exported and covered
- [x] `npm run bundle` run; bundled copies match the shared source
- [x] `npm test` green — 1,222 tests, up from 1,193
