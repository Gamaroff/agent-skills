...
A third probe table in qa-fix Step 3.5, one bullet in the shared cycle-2 refute directive (edited
...
§ *Step 3b* step 2, after the `• Reconnect …` bullet and before `Review the COMBINATION`:

```
     • Identity rules    — for every change to a dedupe, cache or record key, a normaliser or an
                           equality predicate: find one pair that must be the same and one that must
                           differ. A key changed to fix one direction has usually broken the other.
```
...
Mutation proofs: edit one block only (parity red); delete the bullet from both (bullet red); delete
the Should-not-merge row (qa-fix red). `cp` snapshot, mutate, run, restore.
