...
| 7. finalise                | ✅ Done    | `task.106.dod.{N}.*.md`; task `status: accepted`                       | `task.106.dod.1.*.md` — **ACCEPTED**, every decision-matrix column passes. CI 5/5 on `008f95aa` == HEAD at verification time. DoD + canonical summary posted to PR #381; issue #380 commented `done` and closed; task-registry row ticked `accepted`. Status flipped `ready-for-review → accepted` in frontmatter and body | —                    |
| 8. commit-changes          | ✅ Done    | All artifacts committed and pushed                                     | `ac087b40` (finalise artifacts + registry tick) and `eb6b2932` (DoD status-header reconciliation — see Issues Log). Both pushed to `origin/feature/task.106.pr-comment-plain-language-lead` | —                    |
