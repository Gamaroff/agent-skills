...
| 5–6  | `qa-story` → `qa-fix` → `review-pr` | QA review produces a gate file (`PASS` / `CONCERNS` / `FAIL` / `WAIVED`). `qa-fix` runs on an **open finding** — `FAIL`, or any gate with an open `top_issues[]` entry; a `CONCERNS` with no open finding is a reservation, not a queue, and hands to `review-pr` like a `PASS`. Up to 5 cycles. **Step 5c is the loop's exit gate**: a `PASS`/`WAIVED` gate hands to `review-pr`, which reviews the PR against the story. `REQUEST CHANGES` returns to `qa-fix` and consumes a cycle from the same 5-cycle budget; `APPROVE`/`CONCERNS` exit to Step 7. Leaves `story.{E}.{S}.pr-review.{n}.{name}.md`. |
