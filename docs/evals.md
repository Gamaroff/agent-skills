...
npm run eval:full-flow:sdk     # one live scenario via Claude SDK (needs ANTHROPIC_API_KEY)
```
...
### 3. "I want to run one full-flow scenario in replay mode"

```bash
npm run eval:full-flow                     # scenario 01-happy-task
node evals/full-flow/runner.mjs evals/full-flow/scenarios/03-task-id-collision
```
...
Loops scenarios 01–04. Scenario 05 (live tracker) is skipped unless creds are present.
...
```bash
npm run eval:full-flow:sdk                 # scenario 01 against live model
DRIVER=claude-sdk node evals/full-flow/runner.mjs evals/full-flow/scenarios/02-happy-story
```
...
### 7. "I want to run scenario 05 — live Jira / GitHub round-trip"
...
```bash
DRIVER=claude-sdk node evals/full-flow/runner.mjs evals/full-flow/scenarios/05-tracker-payload-live
```
...
| Full-flow scenario | `evals/full-flow/scenarios/<name>/` (see `evals/full-flow/README.md#adding-a-scenario`) |
| Agent backend (Gemini, Goose, Aider, …) | `evals/full-flow/drivers/<name>.mjs` (see `evals/full-flow/README.md#adding-a-driver-for-another-agent`) |
| Structural assertion fn | `evals/full-flow/assertions.mjs` + register in `runner.mjs` switch + test in `tests/assertions.test.mjs` |
...
| **L4 Full-flow** | `evals/full-flow/` | End-to-end artefact shape (frontmatter, sections, citations, tracker payload, halt logs) | `npm run eval:full-flow:all` |

L1–L3 + L4 replay run on every push. Live drivers (`claude-sdk`, `claude-cli`) and scenario 05 are `workflow_dispatch` only.
...
Add a driver: drop `evals/full-flow/drivers/<name>.mjs` implementing the contract in `drivers/types.mjs`. No scenario/assertion changes needed.

### Scenarios

| Scenario | Skill | Covers |
| --- | --- | --- |
| `01-happy-task` | create-task | All 11 mandatory sections, ≥2 citations, frontmatter, tracker payload shape |
| `02-happy-story` | create-story | Required template sections, ≥2 citations, sprint-status merge |
| `03-task-id-collision` | create-task | HALT — pre-existing task.42 untouched, no new file, halt.log emitted |
| `04-story-missing-core-config` | create-story | HALT — no core-config.yaml → no story file, halt.log emitted |
| `05-tracker-payload-live` | create-task | **Live** Jira/GitHub round-trip; needs live driver + creds; cleans up after itself |
...
npm run eval:full-flow         # L4: scenario 01, replay
npm run eval:full-flow:all     # L4: every scenario, replay
npm run eval:full-flow:cli     # L4: scenario 01, DRIVER=claude-cli
npm run eval:full-flow:sdk     # L4: scenario 01, DRIVER=claude-sdk
```
...
- `evals/full-flow/README.md` — driver contract, scenario layout, sabotage-verify workflow
- `evals/full-flow/scenarios/05-tracker-payload-live/README.md` — live tracker env contract, receipt shape, safety notes
- `evals/full-flow/drivers/types.mjs` — `AgentDriver` JSDoc contract
