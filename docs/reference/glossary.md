...
| **Diminishing-returns exit** | The QA loop's third exit, beside a clean gate and `MAX_ITER`. Fires only when **all three** of its conditions hold — two consecutive zero-HIGH gates, every finding naming a file inside `qa.testArtifactGlobs`, and nothing implicating product behaviour — and **exits cleanly**: the loop finished working. Every condition fails closed; see [QA Flow → How the loop ends](../runbooks/qa-flow.md#how-the-loop-ends). The opposite of the Convergence check, which fires when HIGH findings remain and stop falling and **escalates**. Hands to Step 5c exactly as a `PASS` does. Which paths count as machinery is `qa.testArtifactGlobs`. |
