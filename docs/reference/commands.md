...
| `/qa-next` | Select the first `⬜ untested` row of the owner's UAT tracker, resolve the story's ACs to checklist items, execute them against the configured environment, write the run file, record 🟡 pass / ❌ fail (bug filed) / ⏸ blocked, commit. Never writes ✅ accepted — that is the owner's `uat-status.mjs --accept` | [`qa-next` README](../../skills/qa-next/README.md) |
| `/qa-next --dry-run` | Read-only preview of the story `--next` would select and the checklist it would run | [`qa-next` README](../../skills/qa-next/README.md) |
