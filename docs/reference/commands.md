...
| `/qa-next` | Select the first `⬜ untested` row of the owner's UAT registry — one row per **user function**, not per story — resolve it to checklist items (authoring them when none exist), execute them against the configured environment, write the run file, record 🟡 pass / ❌ fail (bug filed) / ⏸ blocked, commit. Never writes ✅ accepted — that is the owner's `uat-status.mjs --accept` | [`qa-next` README](../../skills/qa-next/README.md) |
| `/qa-next <id>` | The same protocol against a **named** registry row — `/qa-next D.2` — whatever state that row is in. Re-test a ❌ after the fix lands; regression-test a ✅ when the code beneath it changes. Each run writes a new file in `runs/<id>/`; a repeat failure re-links the open bug rather than filing a second. A pass, blocked or n/a leaves an ✅ in place — only a fail moves it | [`qa-next` README](../../skills/qa-next/README.md) |
| `/qa-next --dry-run` | Read-only preview of the function `--next` would select and the checklist it would run. `/qa-next <id> --dry-run` previews a named row instead | [`qa-next` README](../../skills/qa-next/README.md) |
