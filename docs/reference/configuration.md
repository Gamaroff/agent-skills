...
subagents: # optional — how long a pipeline waits on a dispatched subagent
  ...
  # and performing the pass inline. Recorded as `killed at N minutes`, never
  # `stalled`. Output-file size is never used as a liveness signal — only the
  # harness's completion notification and this clock are.
  wallClockMinutes: 10

developNext: # optional — develop-next roadmap orchestrator
  roadmapPath: docs/development/project-completion-roadmap.md
  baseBranch: develop
  qualityGateCommand: npm run ci # merge gate run on every branch before gh pr merge
  mergeStrategy: merge # merge | squash | rebase

qaNext: # optional — qa-next UAT loop orchestrator
  trackerPath: docs/qa/uat-tracker.md
  baseUrl: http://localhost:5173 # the app under test — required for a run
  ...
  baseBranch: develop
  commit: true # commit + push the run file, tracker, checklist and bug
  maxItemsPerStory: 12
...
| `qaNext.trackerPath`                             | path                            | `docs/qa/uat-tracker.md`                         | The owner's UAT tracker — one row per accepted story. `uat-surfaces.json` (epic → surface map) is read from the same directory. |
| `qaNext.baseUrl` / `qaNext.apiUrl` / `qaNext.healthPath` | URL / URL / path        | (none) / (none) / `/`                            | The environment `/qa-next` exercises. `baseUrl` is required for a run; `apiUrl` enables HTTP probes; preflight `curl`s `baseUrl + healthPath` (and `apiUrl`) and stops with `env-unreachable` on failure. |
| `qaNext.envLabel`                                | string                          | `local`                                          | Names the environment in run files: `runs/<date>-<envLabel>-<id>.md`. |
| `qaNext.personasDoc`                             | path                            | (none)                                           | Document listing the test accounts and which surface each exercises; read when resolving a story to items. |
...
| `qaNext.maxItemsPerStory`                        | int                             | `12`                                             | Cap on authored checklist items per story; a story needing more is flagged as too big for one iteration. |
