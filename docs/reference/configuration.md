...
qaNext: # optional — qa-next UAT loop orchestrator
  registryPath: docs/qa/uat-registry.md # one row per user function; uat-surfaces.json beside it
  ...
  baseBranch: develop
  commit: true # commit + push the run file, registry, checklist and bugs
  maxItemsPerFunction: 12
...
| `qaNext.maxItemsPerFunction`                     | int                             | `12`                                             | Cap on authored checklist items per function; a function needing more is really two rows, and the report says so. |
