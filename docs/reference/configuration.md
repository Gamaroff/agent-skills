...
| `developBatch.worktreeSeedPaths` | array | `[]` | Gitignored paths copied from the main tree into each fresh worktree. A `git worktree add` carries no gitignored files, and a missing runner config usually degrades *silently* rather than failing. **Entries are repo-relative paths and must be copied path-preservingly** (`mkdir -p "$dir/$(dirname "$p")" && cp "$p" "$dir/$p"`, or `rsync -R`) — a flattening `cp "$p" "$dir/"` silently clobbers when two entries share a basename (e.g. `.env` and `apps/api/.env`) and never creates the nested location that needed the file. |
