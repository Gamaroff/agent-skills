...
| `access.tracker`                                 | `full` \| `read-only` \| `approve` \| `command` \| `manual` | `full`                | How much access the agent has to the tracker — a separate axis from `tracker`, which says only *which* tracker. Also settable via `AGENT_SKILLS_ACCESS_TRACKER`; config and env are read independently and the **more restrictive** wins, so an env var can lock a run down but never escalate it. An unrecognised value halts. **Write it in the canonical block form** (`access:` at column 0, an indented `tracker:` beneath) — the no-dependency awk reader used on a host without `pyyaml` does not see a merge key, an anchor, a quoted key, or a mapping-valued child, and reads any of those as *unset*, i.e. `full`. See [Platform Detection](../../shared/resources/platform-detection.md) → *Known limit*                                                                                      |
