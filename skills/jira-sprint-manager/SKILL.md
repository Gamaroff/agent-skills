...
| `list-sprints.sh <board_id> [state]` | List sprints by state (`future,active` default). | JSON array `[{id, name, state, startDate, endDate, goal}]` |
| `get-active-sprint.sh <board_id>` | Active sprint(s) for board. Empty array = none. | JSON array |
...
| `discover-sp-field.sh` | Print the Story Points custom field ID. | `customfield_NNNNN` |
...
- **Story Points field:** defaults to `customfield_10026` (team-managed). For company-managed projects run `./scripts/discover-sp-field.sh` and export `JIRA_SP_FIELD`.
...
1. **Find the sprint.** If user only gave board ID, run `list-sprints.sh <board_id> future` and ask which to start.
2. **Check for conflicts.** Run `get-active-sprint.sh <board_id>`. If non-empty, warn user unless parallel sprints are explicitly allowed.
...
3. **Decide destination.** If incomplete list is non-empty, present keys and ask: move to **backlog** or to **next future sprint**? If next sprint, run `list-sprints.sh <board_id> future` to surface candidates.
...
Optional:
- `JIRA_SP_FIELD` — Story Points field ID (default `customfield_10026`). Discover with `./scripts/discover-sp-field.sh`.
