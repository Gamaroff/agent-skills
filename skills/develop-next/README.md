...
- **Review cadence:** you review once per **epic** (the epic→base merge is followed by a stop) instead of once per story. Merges, ticks, and auto-answered decisions are all recoverable from the run report, each story's implementation report (Decisions Log), and the roadmap Change Log.
- **Planning gaps:** a roadmap row that names `/create-story` or `/create-epic` stops the loop *before* authoring — those skills are interactive and their output needs human review. Run them in an attended session, then restart the loop.
