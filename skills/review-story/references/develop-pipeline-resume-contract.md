...
3. **Offer the grant.** When the halt snapshot's `halt_reason` matches `loop-limit|not-converging`,
   ...
   Source: `references/grant-qa-cycles.sh`; suite: `grant-qa-cycles.test.sh`. Everything it
