...
5. **Configuration Accuracy**:
   ...
   - **Relative Markdown links resolve from the document's own directory, not from wherever the
     ...
     `docs-link-check` asks. On task.139 the task document (the story sibling is the same shape) quoted `develop/SKILL.md` verbatim — including
     ...
     skill); it resolves against the **tracked** tree, skips fences and code spans, and exits 1 with
     one `✖ file:line → target` per dead link:

     ```bash
     node references/doc-links.js --file "$STORY_FILE" --json
     ```
