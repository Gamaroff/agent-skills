...
the review-driven path: 5c runs when the gate reached it (any of the QA loop's five accepting routes, §5c), so a `REQUEST CHANGES`
...
- Within same severity, sort by source: gate > pr-review > report > bug. (`pr-review` ranks above
  ...
  the gate that sent the run to 5c carries no open fix target on any of §5c's five routes.)
