...
| Surface | Operations |
|---|---|
| **Filesystem** | task file (read/append `[x]`), implementation report (`task.{id}.implementation.{N}.*.md`), review report, plan file, gate file (`.yml`), QA report, DoD summary, lock file `.claude/state/develop-pipeline.lock` |
...
- **Files mutated**: task file (status frontmatter, `[x]` task ticks), implementation report (Pipeline Progress table, Decisions Log, Issues Log, QA Iteration History), lock file (`current_step`, `pr_url`)
...
1. **QA loop dominates wall-clock.** Up to 5 cycles of `/qa-task` + `/qa-fix` + commit + push. Each `/qa-task` may itself spawn parallel agents (suppressed in `lite` mode).
...
sequenceDiagram
    ...
    User->>H: /develop-task path

    rect rgba(99,102,241,0.18)
    note over H: Phase 0 — Resolve & Prepare
    H->>EX: resolve task file/dir
    EX-->>H: absolute path, task id, name
    H->>FS: read task (status, phases, risk)
    ...
    note over H: Step 2 — review-task
    H->>FS: lock.current_step=2
    H->>RT: review task (autonomous mode)
    RT->>FS: write review report and mutate task status
    RT-->>H: outcome (LGTM / NEEDS REVISION / REQUIRES REWORK)
    alt NEEDS REVISION or REQUIRES REWORK
        H-->>User: HALT (task not ready — surface findings)
    else LGTM or critical fixes auto-applied
        note over H: continue
    end
    ...
    loop until status Ready for Review or MAX_ITER
        ...
        DV->>FS: edit code, tick phases, set status
    ...
    loop until PASS or 5 cycles
        H->>QT: qa-task with lite mode flag if set
        QT->>FS: write gate yml and qa report (co-located in task dir)
        QT-->>H: gate result
    ...
    H-->>User: Task Development Complete (branch, PR, QA cycles, report)
```
...
| From → To | Object |
|---|---|
| User → Harness | task path / `#N` / Jira key |
| Harness → Explore | resolution query / surface-map query |
| Explore → Harness | absolute path; file→1-line summary list |
| Harness → `/create-branch` | task path, base branch (Q1) |
| Harness → `/review-task` | task path (autonomous mode — auto-answers critical fixes + status update) |
| Harness → `/develop` | task path, surface map, plan file, iteration hint |
| `/develop` → Harness | (returns; harness re-reads task for status/[x]) |
| Harness → `/create-pr` | `--base`, `--exclude {report}`, `--issue N` (GitHub only) |
| `/create-pr` → Harness | PR URL |
| Harness → `/qa-task` | task path, lite-mode flag |
| `/qa-task` → Harness | gate file path, gate result |
| Harness → `/qa-fix` | gate file path |
| Harness → `/finalise` | task path |
...
- `status: accepted` in task frontmatter
