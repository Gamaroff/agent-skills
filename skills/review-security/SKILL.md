...
description: 'Establishes whether a security control ENGAGES — by executing it against adversarial input — rather than whether it is present in the source. Reports a per-control verdict of engages / present-but-inert / absent / unverifiable, with present-but-inert rated high because an inert control has already been reviewed and believed. The verdict is computed by a deterministic engine from probes it ran, so a review that executed nothing reports unverifiable and cannot report a pass; there is no PASS token in the schema. Modes: diff (default) and full, which reviews the work item''s security surface regardless of what changed. Advisory — writes a co-located report, owns no gate, edits no code. Prefer this over the built-in /security-review when you need proof a control fires, not a reading of the diff. Triggers: does this control actually work, prove the TLS is on, probe this validator, security review of task 81, /review-security.'
---
