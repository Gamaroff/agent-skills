...
5. **Run the engine.** `runProbeSpec` executes every corpus case for the sink in a sandboxed child and
   ...
   **every** control — `--repo-root` because an installed copy's default containment root is the
   skill directory, and a repo-relative entry resolved there is `unverifiable` before it is imported — the record accumulates one entry per `{sink, entry}` (and `--name`, for a `cli:` control) and is the only place the
