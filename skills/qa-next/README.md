...
`--check` enforces: 🟡/✅/❌ carry a run link that resolves; **every bug link the tool wrote resolves, on any row** — ❌ must additionally carry one, and a scheme-qualified or anchor-only link is treated as a human's prose reference and skipped; ⏸/➖ carry a note; every row id matches its section letter and its header's cell count; every `Stories` id is a real story.
