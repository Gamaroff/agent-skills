...
6. **Post QA Summary to PR** — **CRITICAL / BLOCKING**: This step is mandatory. The review is NOT complete until the PR comment is confirmed posted. Do not skip, defer, or treat as optional.
   ...
   **Verify the comment was posted**: Confirm `gh pr comment` exited with code 0. If it fails, report the error to the user and retry or provide the comment body for manual posting. Do NOT proceed to step 7 until the comment is confirmed posted.
