...
| `gh` | `pr` / `issue` / `repo` / `run` / `release` / `workflow` + `list` / `view` / `status` / `checks` / `diff` with list/view flags (`--json` `--jq` `--state` `--limit` …; never `--web`); `api <path>` first — a path on the authenticated host, never a URL (`://` and a leading `//` are refused: gh sends an absolute endpoint to that host as-is) — then only read-shaping flags (`--jq` `--paginate` `--cache` `--template` `-i` `-H` …) — no `--hostname`, no method or field flags |
