...
`npx prettier --write=.`, `npx tsc` (no `--noEmit`), `npx vitest` (no `--run`), `npx prettier --config=x.mjs .`, `npx eslint -c <any in-repo .js> .`, `npx eslint -f ./x.js .`, `npx mocha -R ./x.js`, `npx mocha init out`, `npx vitest init browser`, `npx mocha -R index t`, `npx vitest --run --reporter=html x`, `npx jest --reporters=jest-junit`, `jq -n env`, `jq -n null//env`, `find -fprint`, `date 0101120026`, `rm`, `sudo`,
