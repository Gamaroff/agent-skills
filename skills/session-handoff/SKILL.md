...
| `node` | allow-listed leading flags (`--test`, `--test-concurrency=`, `--test-reporter=` **built-in names only**, `--enable-source-maps`, `--no-warnings`, `--max-old-space-size=` …), then **one of an exact list of read-only entry points**, each with its own spec for what may follow it — the `NPM_SCRIPTS` discipline applied to scripts: `develop-next/scripts/select-next.mjs` (`--lint`, `--batch`, the three `--*-registry`/`--roadmap` relative paths; no positional) and `observation-log.js` (under any skill's bundled `references/` directory, or at its shared source; **read verbs only** — `doctor` `scan` `queue` `next-id` `families` — with `--json` `--quiet` `--audit` and a `--workspace` / `--audit-root` that may be absolute, because the log lives outside the repo; `write` `init` `set-status` `archive` `checkpoint` refused). Each is matched at this repository's `skills/…` path and at a consumer's `.agents/skills/…` path. Until gate 7 **any** relative script passed with its whole tail passed through, and `node node_modules/prettier/bin/prettier.cjs --write x` rewrote a file through read mode (bug.8, executed) — the repo's own writers were one spelling away from the `npm run` forms that refuse them. In `--test` mode there is no script and node keeps parsing its own options, so every later dash token is held to the same list (`node --test x/ -r pre.js` preloads `pre.js` otherwise) |
...
`node node_modules/prettier/bin/prettier.cjs --write x`, `node …/registry-tick.js`, `node …/observation-log.js write`, `python3 skills/create-skill/scripts/generate_catalog.py`,
