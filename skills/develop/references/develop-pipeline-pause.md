...
└──────────────────────────────────────────────────────────────────┘
                  ...
                  ▼
┌──────────────────────────────────────────────────────────────────┐
│ Hook CLAIMS the lock (atomic mv → .pausing.<pid>; losers noop): │
│   1. Append "Paused — Context Compaction" to implementation report│
│   2. git commit + git push (best-effort)                         │
│   3. PR comment via tracker_write (marker, lead, --body-file;    │
│      find-by-marker → edit in place → else create)               │
│   4. Issue comment via tracker-comment.js (lead, marker, gate)   │
│   5. rm the claimed lock                                         │
...
0. **The claim (one pause per lock).** The first thing the hook does is `mv` the lock to `develop-pipeline.lock.pausing.<pid>`. `mv` on one filesystem is atomic, so of N concurrent invocations exactly one owns the lock; every other invocation's `mv` fails and takes the same empty-signal noop path a stray re-fire always took, producing **nothing** — no snapshot, no report block, no comment. This is what makes the hook safe to register twice (task.120: a `settings.json` carried it under two path spellings and the host fired both, so every side-effect below was produced twice). The winner then sweeps any *other* `.pausing.*` file — a loser never owns one, so anything else there was left by a run the harness killed mid-flow — and addresses the claimed name for everything that follows. (A run killed between its claim and its snapshot has its state nowhere but that claim file — see item 1 for how the resume detector reads it.) Two consequences worth knowing: (a) during the hook's own runtime the lock is briefly absent under its original name, so a `Stop` hook firing inside that window sees no pipeline — the intended end state after a pause anyway; (b) `PIPELINE_LOCK` must sit on the same filesystem as its own directory, which a path is by definition.
1. **Snapshot-before-removal (resume guarantee).** As soon as the lock is claimed — *before* the `trap 'rm -f "$LOCK"' EXIT` is armed and before any explicit `rm` — write `develop-pipeline.last-halt.json` (co-located with the lock). It is a **superset of the lock** with `paused_at`, `pause_reason: "precompact"`, and `halt_step` (= the lock's `current_step`). When `jq` is unavailable this degrades to a verbatim `cp` of the lock (still preserving `current_step`). This is what makes a *killed* hook recoverable: every exit path after the snapshot — clean finish OR harness SIGTERM/timeout mid-flow — leaves it on disk, so the pipeline does not end up both unlocked **and** un-resumable. **One window is not covered by this item alone**: a kill between the claim (item 0) and this write leaves the state only under the claimed name, `develop-pipeline.lock.pausing.<pid>` — the lock byte for byte, renamed. The Phase 0a resume detector therefore reads `.pausing.*` alongside `last-halt.json` whenever the lock is absent, choosing by document (`task_or_story_directory`) and then by age — a fixed snapshot-first order would let a stale snapshot, which nothing consumes on resume, shadow the fresher claim (`current_step` is the resume point, as in a `cp`-degraded snapshot). The claim file is swept only by the *next* pause, which by then has claimed a newer lock, so the fallback is never raced away. The implementation report remains the primary recovery record throughout, as it always was. Phase 0b's resume detector consumes this snapshot when no active lock is present.
2. Append a "Pipeline Paused — {timestamp}" block to `report_path` with skill, branch, last step boundary, PR URL, tracker info, and resume instructions.
3. `git add <report> && git commit -m "docs(<skill>): pipeline paused at step <N> — context compaction imminent" && git push origin HEAD`.
4. The PR comment, if `pr_url` is non-empty and both `gh` and `node` are on PATH. The body file (`.claude/state/precompact-pr-comment.step-<N>.md`) opens with the marker `<!-- agent-skills-comment:pipeline-paused-<N> -->` — the same family `tracker-comment.js` writes on the issue arm — then the `pipeline-paused` lead (rendered by `stakeholder-summary-cli.js`), then `---`, then the paused notice. The post is **find-by-marker → edit in place → else create**, the `finalise` Step 6c recipe: `gh pr view --json comments` is searched for a body starting with the marker, and a hit is `gh api -X PATCH …/issues/comments/<id> -F body=@<file>`, a miss is `gh pr comment <pr_url> --body-file <file>`. Both arms run through `tracker_write` (sourced from `resolve-platform.sh` beside the hook) so a declared `access.tracker` restriction is honoured — the mutation is recorded in the deferred-mutation journal instead of performed. The search is a read and is deliberately outside the gate. It is also a *partial* read — `gh pr view --json comments` does not page — accepted here as `finalise` accepts it; a PR busy enough to page the marker out of view is not a case the pause hook has met. If the resolver or the lead cannot be loaded the arm fails closed: nothing is posted and the signal says so.
5. `tracker-comment.js --issue <tracker_issue> --stage pipeline-paused-<step> --body-file .claude/state/precompact-issue-comment.step-<N>.md` if `tracker_issue` is set and `node` is on PATH. One call: the engine resolves the tracker, renders the lead, adds the idempotency marker (scoped by step) and applies the access gate. Its `reason` is surfaced in the signal.
6. `rm -f` the claimed lock.

> **A second pause at the same step.** A pause at a *later* step posts fresh comments on both arms (the marker is step-scoped). A repeat at the *same* step behaves differently per arm, and a reader should know which: the PR arm **edits** the earlier comment in place, so the PR shows the *latest* pause at that step, not the first; the issue arm reports `already` and posts nothing. Nothing is lost — the implementation report keeps every pause block, and that is the record.
