...
- **Story branches always target the epic branch** — both Q1 (base) and Q2 (PR target) are auto-set, never asked.
...
- **Epic branch resolution + creation** — Phase 0a extracts `epic:` frontmatter, derives `EPIC_BRANCH`, and creates it from `develop` if absent. Q1 base branch is never asked (auto = `EPIC_BRANCH`).
...
flowchart TD
    ...
    P0d -- no --> P0dconf[0d confirm epic-branch creation<br/>auto Q2 = EPIC_BRANCH]
    P0d -- yes --> P0drisk[0d Q3 only if risk_level: high]
    P0dconf --> P0drisk
    P0drisk --> P0e[0e create implementation report]
