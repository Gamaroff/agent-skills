...
**Restore the lock before anything advances it (task.124 QA cycle 2, CR-2).** A resume skips Step 1, which is the lock's only ordinary writer, and every terminal HALT and PreCompact pause removed the lock and left a superset of it behind. When the resume detector's `source` is `halt_snapshot` or `orphaned_claim` and the operator chooses **Resume** — **and, in `develop-task` or `develop-story`, the snapshot's `halt_reason` is not `loop-limit|not-converging`** (on those the grant restores: `grant-qa-cycles.sh` calls `--restore` only after its never-lower guard passes, so a declined or refused grant leaves nothing behind — QA cycle 3, CR-1; see the resume contract's Re-entry. `develop-bug` has no grant prompt, so a `develop-bug` snapshot runs the command below whatever its `halt_reason` reads — QA cycle 5, CR-1) — run, before Phase 0b verification, before any step banner:
...
**Lite mode detection**: `PIPELINE_MODE` is resolved from the three inputs the Lite-mode + always-load detector (Agent 3, dispatched in **0a-parallel**) reports by **reading the document** — that is Agent 3's normal path, and there is no lite-mode CLI in the tree (`references/develop-pipeline-lite-mode.md` is the contract, not an executable). The Aggregation block then computes `PIPELINE_MODE` from those reported booleans (`risk_ok ∈ {low,absent} AND phase_count < 3 AND single_module`). What is forbidden is judging `pipeline_mode` **by impression**: report the three inputs as separate fields and let the AND decide. **If Agent 3 was not dispatched** (Phase 0 run inline), derive the same three inputs from the document yourself, exactly as its prompt says, and record that you did. See `references/develop-pipeline-lite-mode.md` for trigger conditions, `PIPELINE_MODE=lite` behaviour, and the directive format passed to the QA skill.
...
[`references/implementation-report-template.md`](implementation-report-template.md) — the
...
**Why the template is not inlined here.** `references/report-lint.js` derives the sections a
