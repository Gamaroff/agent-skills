...
const SHARED_ON_STOP = path.join(
  REPO_ROOT,
  "shared",
  "resources",
  "develop-pipeline-on-stop.sh",
);
const SHARED_INSTALL = path.join(
  REPO_ROOT,
  "shared",
  "resources",
  "develop-pipeline-install-hooks.sh",
);
const HOOKS_DOC = path.join(
  REPO_ROOT,
  "shared",
  "resources",
  "develop-pipeline-hooks.md",
);
const STEP0_SHARED = path.join(
  REPO_ROOT,
  "shared",
  "resources",
  "develop-pipeline-step-0-resolve-and-prepare.md",
);
const STEP3_SHARED = path.join(
  REPO_ROOT,
  "shared",
  "resources",
  "develop-pipeline-step-3-develop-loop.md",
);
const STEP8_SHARED = path.join(
  REPO_ROOT,
  "shared",
  "resources",
...
test("#2 — develop-story SKILL.md documents Step Transition Protocol", async () => {
  ...
  assert.match(
    ...
    "Step Transition Protocol heading missing",
  );
  assert.match(
    content,
    /lock-update Bash call is the binding signal/i,
    "binding-signal rationale missing",
  );
  assert.match(
    content,
    /do NOT print "Returning to pipeline orchestrator"/,
    "explicit forbidden-prose list missing",
  );
});
...
test("#2b — develop-story Setup section names both hooks and points to install script + canonical doc", async () => {
  ...
  assert.match(
    content,
    /PreCompact/,
    "Setup section must name the PreCompact hook",
  );
  assert.match(content, /\bStop\b/, "Setup section must name the Stop hook");
  assert.match(
    content,
    /install-hooks\.sh/,
    "Setup section must reference the install script",
  );
  assert.match(
    content,
    /references\/develop-pipeline-hooks\.md/,
    "Setup section must link the canonical hooks doc",
  );
});
...
test("#2b — canonical on-stop.sh honours stop_hook_active loop protection", async () => {
  ...
  assert.match(
    content,
    /develop-pipeline\.lock/,
    "must reference pipeline lock file",
  );
  assert.match(content, /current_step/, "must check current_step field");
  assert.match(
    content,
    /decision.*block/i,
    "must emit decision:'block' when blocking",
  );
...
test("#2b — develop-{story,task} on-stop.sh wrappers are byte-identical and exec the canonical", async () => {
  ...
  assert.equal(
    taskHook,
    storyHook,
    "Both on-stop.sh wrappers must be identical",
  );
  assert.match(
    storyHook,
    /exec .*references\/develop-pipeline-on-stop\.sh/,
    "wrapper must exec the bundled canonical",
  );
});
...
test("#2b — canonical install-hooks.sh contract", async () => {
  ...
  assert.match(
    script,
    /\.agents\/skills\/develop-story/,
    ".agents path candidate (npx skills add)",
  );
  assert.match(
    script,
    /\.claude\/skills\/develop-story/,
    ".claude path candidate (symlink/monorepo)",
  );
  assert.match(
    script,
    /already registered/,
    "must be idempotent (skip on duplicate)",
  );
...
test("#2b — develop-{story,task} install-hooks.sh wrappers are byte-identical and exec the canonical", async () => {
  ...
  assert.equal(
    taskScript,
    storyScript,
    "install-hooks.sh wrappers must be byte-identical across develop-{story,task}",
  );
  assert.match(
    storyScript,
    /exec .*references\/develop-pipeline-install-hooks\.sh/,
    "wrapper must exec the bundled canonical",
  );
});
...
const SHARED_LOCK_COOP = path.join(
  REPO_ROOT,
  "shared",
  "resources",
  "pipeline-lock-cooperation.md",
);
...
test("#2d — on-skill-return.sh hook scripts are gone (canonical, wrappers, bundled)", async () => {
  const candidates = [
    path.join(
      ...
      "shared",
      "resources",
      "develop-pipeline-on-skill-return.sh",
    ),
    path.join(
      REPO_ROOT,
      "skills",
  ...
  for (const p of candidates) {
    assert.ok(
      await fileAbsent(p),
      `obsolete hook script must not exist: ${path.relative(REPO_ROOT, p)}`,
    );
  }
});
...
test("#2d — canonical install-hooks.sh registers no PostToolUse hook and de-registers the obsolete one", async () => {
  ...
  assert.doesNotMatch(
    script,
    /POSTTOOLUSE_CMD/,
    "must not define a PostToolUse command",
  );
  assert.match(
    script,
    /unpatch_hook "PostToolUse" "on-skill-return/,
    "must de-register the obsolete PostToolUse/on-skill-return hook",
  );
});
...
test("#2d — setup-consumer.sh registers no PostToolUse hook and de-registers the obsolete one", async () => {
  ...
  assert.doesNotMatch(
    script,
    /_patch_hook "PostToolUse"/,
    "must not register a PostToolUse hook",
  );
  assert.match(
    script,
    /_unpatch_hook "PostToolUse" "on-skill-return/,
    "must de-register the obsolete PostToolUse/on-skill-return hook",
  );
});
...
test("#2e — canonical install-hooks.sh emits ${CLAUDE_PROJECT_DIR}-prefixed commands, not bare relative paths", async () => {
  ...
  assert.match(
    script,
    /CLAUDE_PROJECT_DIR/,
    "must build the hook command with ${CLAUDE_PROJECT_DIR}",
  );
  assert.match(
    script,
    /PRECOMPACT_CMD=.*CLAUDE_PROJECT_DIR.*on-precompact\.sh/,
    "PreCompact command must be cwd-independent",
  );
  assert.match(
    script,
    /STOP_CMD=.*CLAUDE_PROJECT_DIR.*on-stop\.sh/,
...
test("#2e — pause doc example commands use ${CLAUDE_PROJECT_DIR}, not a bare relative path", async () => {
  const content = await readFile(
    path.join(REPO_ROOT, "shared", "resources", "develop-pipeline-pause.md"),
    "utf-8",
  );
  assert.match(
    content,
    /"command":\s*"bash \\"\$\{CLAUDE_PROJECT_DIR\}\//,
    "settings.json example must model the cwd-independent form",
  );
  assert.doesNotMatch(
    content,
    /"command":\s*"bash \.agents\/skills/,
    "settings.json example must not show a bare relative command",
  );
});
...
test("#2d — orchestrator SKILL.md files no longer reference PostToolUse/on-skill-return", async () => {
  for (const [label, skill] of [
    ...
    assert.doesNotMatch(
      content,
      /PostToolUse/,
      `${label}: SKILL.md must not mention PostToolUse`,
    );
    assert.doesNotMatch(
      content,
      /on-skill-return/,
      `${label}: SKILL.md must not mention on-skill-return`,
    );
  }
});
...
test("#2d — Step Transition Protocol lists exactly two structural defences (self-advance + Stop)", async () => {
  for (const [label, skill] of [
    ...
    assert.match(
      ...
      `${label}: must say 'Two structural defences'`,
    );
    assert.doesNotMatch(
      content,
      /Three structural defences/,
      `${label}: must not claim three structural defences`,
    );
  }
});
...
test("#2d — pipeline-lock-cooperation.md cooperation order omits the PostToolUse hook", async () => {
  ...
  assert.doesNotMatch(
    content,
    /PostToolUse/,
    "cooperation doc must not reference a PostToolUse hook layer",
  );
});
...
test("#2c — canonical hooks doc exists and catalogues both hooks", async () => {
  ...
  assert.match(
    content,
    /stop_hook_active/,
    "must document stop_hook_active loop protection",
  );
  assert.match(
    content,
    /decision.*block/i,
    "must document decision:'block' return contract",
  );
  assert.match(
    content,
    /current_step/,
    "must document current_step lock field",
  );
  // Required sections (allows for emoji/formatting prefixes before the heading text)
  assert.match(
    content,
    /## Hook catalog/,
    "must have a 'Hook catalog' section",
  );
  assert.match(
    content,
    /## Interaction model/,
    "must have an 'Interaction model' section",
  );
  assert.match(
    content,
    /## Troubleshooting/,
  ...
  assert.match(
    ...
    "must define the contract for adding new hooks",
  );
});
...
test("#1 — step-0 doc has Required-question count check table", async () => {
  ...
  assert.match(
    content,
    /Required-question count check/,
    "count-check heading missing",
  );
  // develop-story now asks exactly 2 questions (Q1 base + Q2 PR target) — same as
  // develop-task; the flat develop flow removed the epic-branch creation prompt.
  assert.match(
    content,
    /`develop-story`.*Q1 \(branch base\) \+ Q2 \(PR target\).*\*\*2\*\*/s,
    "develop-story row must require 2 questions (Q1 base + Q2 PR target)",
  );
  assert.doesNotMatch(
    content,
    /EPIC_BRANCH_EXISTS/,
    "count-check table must not reference the removed epic-branch scenario",
  );
  assert.match(
    content,
...
test("#3+#4 — step-8 doc has BLOCKING post-condition checklist", async () => {
  ...
  assert.match(
    content,
    /Step 8 Completion Checklist.*BLOCKING/i,
    "BLOCKING checklist heading missing",
  );
  // Lock absence check
  assert.match(
    content,
