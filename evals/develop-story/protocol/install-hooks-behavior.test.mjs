...
test(
  "#2e-behavior — migration replaces a legacy bare-relative entry (no duplicate)",
  { skip: !HAS_JQ && "jq not installed" },
  (t) => {
      ...
      JSON.stringify(
        {
          hooks: {
            PreCompact: [
              {
                matcher: "*",
                hooks: [{ type: "command", command: LEGACY_PRECOMPACT }],
              },
            ],
            Stop: [
              {
    ...
    for (const event of ["PreCompact", "Stop"]) {
      ...
      assert.equal(
        cmds.length,
        1,
        `${event}: exactly one entry after migration (got ${cmds.length}: ${JSON.stringify(cmds)})`,
      );
      assert.match(
        cmds[0],
        /\$\{CLAUDE_PROJECT_DIR\}/,
        `${event}: migrated entry must be cwd-independent`,
      );
      assert.ok(
        !cmds.includes(LEGACY_STOP) && !cmds.includes(LEGACY_PRECOMPACT),
        `${event}: legacy bare-relative entry must be gone`,
      );
    }
  },
);
...
test(
  "#2e-behavior — fresh install emits exactly one ${CLAUDE_PROJECT_DIR} entry per event, idempotent on re-run",
  { skip: !HAS_JQ && "jq not installed" },
  (t) => {
    const sandbox = makeSandbox(t);
    const settingsPath = path.join(sandbox, "settings.json");
    ...
    for (const event of ["PreCompact", "Stop"]) {
      ...
      assert.equal(
        cmds.length,
        1,
        `${event}: exactly one entry on fresh install`,
      );
      assert.match(
        cmds[0],
        /^bash "\$\{CLAUDE_PROJECT_DIR\}\/.*on-(stop|precompact)\.sh"$/,
        `${event}: expected cwd-independent command form`,
      );
    }
    ...
    assert.equal(
      ...
      first,
      "re-running the installer must be a byte-identical no-op",
    );
  },
);

test(
  "#2e-behavior — emitted command resolves from a subdirectory; legacy bare-relative form does not",
  { skip: !HAS_JQ && "jq not installed" },
  (t) => {
    const sandbox = makeSandbox(t);
    const settingsPath = path.join(sandbox, "settings.json");
    ...
    assert.match(
      ...
      "the ${CLAUDE_PROJECT_DIR} command must resolve and run from a subdirectory",
    );
