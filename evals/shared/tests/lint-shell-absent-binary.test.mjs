...
function which(name) {
  ...
  assert.equal(
    r.status,
    0,
    `${name} must be resolvable to build the sandbox PATH`,
  );
...
function sandboxPath(extra = () => {}) {
  ...
  for (const tool of ["bash", "git", "grep", "sed"])
    symlinkSync(which(tool), path.join(bin, tool));
  extra(bin);
  return bin;
}
...
test("lint-shell.sh — shellcheck absent: skip message on stdout, exit 0", () => {
  ...
  try {
    ...
    assert.match(
      r.stdout,
      new RegExp(SKIP_MESSAGE),
      `skip message missing from stdout:\n${r.stdout}`,
    );
    assert.doesNotMatch(
      r.stdout,
      /shellcheck: clean/,
      "the lint must not report clean when it did not run",
    );
  } finally {
...
test("lint-shell.sh — shellcheck present: the skip branch is not taken", () => {
  ...
  try {
    ...
    assert.doesNotMatch(
      r.stdout,
      new RegExp(SKIP_MESSAGE),
      "skip branch taken although shellcheck was on PATH",
    );
    assert.match(
      r.stdout,
      /shellcheck: clean/,
      `lint did not run to completion:\n${r.stdout}`,
    );
  } finally {
    rmSync(bin, { recursive: true, force: true });
  }
