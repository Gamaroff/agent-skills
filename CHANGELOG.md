...
- **`uat-status.mjs` owns `/qa-next`'s run state file — `--state-init`, `--state-get`,
  ...
  A run resumed at `executed` records its file first (`--run-path`, then `--state-set runFile`), which
  keeps the value exact. Otherwise, from `executed` on, the file v0.51.0 wrote is excluded by its name
...
- **The Change Log engine now ships with every skill whose prose runs it (task.139, obs #152).**
  `document-change-log.md` § *How a writer appends a row* is the one statement of how a Change Log
  row is written — a `node -e` one-liner that `require`s `./.agents/skills/…/references/change-log.js`
  under the rule "append through `change-log.js`, never by text search". Its skill segment was the
  bare `{skill}`, which the bundler skips by design (a placeholder names no skill; following it
  over-matched 38 files), so `develop` instructed the call and its bundle did not carry the module:
  on task.136 it failed `MODULE_NOT_FOUND`, and in every consumer install the `Implemented — N files,
  M tests` row was silently skipped on every `/develop` run, because the contract's own "no regex
  fallback" rule turns a missing engine into a skipped step. The one-liner now spells the writer
  alternation `{develop|finalise}` — the discovery form the bundler already follows out of shared
  text (`create-skill` § UNREACHED) — so `skills/develop/references/change-log.js` is generated by
  `npm run bundle` and no skill outside the alternation gains a copy. New
  `tests/change-log-engine-reachability.test.js` derives the writer population from the prose
  (`skills/*/SKILL.md` saying "through `change-log.js`", floor ≥ 2 so a rewording cannot empty it),
  asserts each member ships the engine byte-identical to the shared source, and asserts the
  alternation equals that population in both directions; three mutants (member dropped from the
  alternation, copy tampered, phrase reworded) each red their own assertion. Eight other skills
