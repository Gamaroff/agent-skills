...
- **The card preflight now catches a section that is a label with nothing under it, and its
  ...
  under it never reached the card — **28 of 120** task documents published `**Functional**` (or a
  run-on of label and bullets) as their entire Success Criteria block, and the preflight passed every
  ...
  with nothing under it is the new **`heading-only`** finding, defined by property (no sentence
  terminator, no list item) rather than by regex, so a plain-text label counts the same way. The
  ...
  documents and asserts zero; reverting the summariser turns it red at 28. **Body diff on next sync:**
