...
- **CI — eval replay `*.log` fixtures now tracked:** the blanket `*.log` `.gitignore` rule excluded committed replay fixtures (`evals/create-task/scenarios/02-id-collision/replay/.eval/halt.log` and `evals/create-story/scenarios/02-missing-core-config/replay/.eval/halt.log`), so `npm run eval:all` passed locally (fixtures present on disk) but failed in CI on a fresh clone (`missing file: .eval/halt.log`). Added a scoped `.gitignore` negation (`!evals/**/replay/**/*.log`) and committed both fixtures.
