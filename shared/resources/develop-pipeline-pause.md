...
- **Restored**: by **`advance-pipeline-lock.sh --restore <doc-dir>`** — the one restore path (task.124, Phase 4; obs #123). It rebuilds the lock from the newest candidate **for this document** — the halt snapshot `last-halt.json` (written by a terminal HALT or by this hook) or an orphaned `.lock.pausing.<pid>` claim — strips `halted_at` / `halt_reason` / `halt_step` / `paused_at` / `pause_reason`, sets `current_step` to the halted step, refuses a candidate for another document, and **consumes** the source. `grant-qa-cycles.sh` (task.123) calls it rather than carrying its own. **A session that continues in place after this hook fires — rather than re-invoking the skill — must run `--restore` before any step advances**: Step 1 is the lock's only ordinary writer and a resume skips it, and `advance-pipeline-lock.sh <n>` with no lock is now an error naming `--restore`, not the silent exit 0 that once left every advance and the Stop hook inert for a whole session. The hook's own signal names the command.
