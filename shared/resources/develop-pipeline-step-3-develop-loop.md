...
- **High-risk gate** (`risk_level: high`): qa-planning skip is silent — the pipeline always auto-skips it (see `shared/resources/develop-pipeline-step-0-resolve-and-prepare.md`; there is **no** Q3 in Upfront Setup). The `/develop` skill presents up to three options: "Run `/qa-planning` now", "Skip, I've already planned", "Skip, low actual risk". Always automatically select **"Skip, I've already planned"** (treat "Skip, low actual risk" as equivalent if that is what develop offers) and log `"high-risk gate: auto-skipped qa-planning"` in the Decisions Log. Never pause for the user at this gate.
...
- **High-risk gate** (`risk_level: high`): qa-planning skip is silent — the pipeline always auto-skips it (see `shared/resources/develop-pipeline-step-0-resolve-and-prepare.md`; there is **no** Q3 in Upfront Setup). The `/develop` skill presents up to three options: "Run `/qa-planning` now", "Skip, I've already planned", "Skip, low actual risk". Always automatically select **"Skip, I've already planned"** (treat "Skip, low actual risk" as equivalent) and log `"high-risk gate: auto-skipped qa-planning"` in the Decisions Log. Never pause for the user at this gate.
